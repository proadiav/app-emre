# Module 5 — Fiche Client Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Créer la page `/customers/[id]` affichant la fiche complète d'un client avec infos, résumé parrainage, ventes, filleuls, bons et actions contextuelles.

**Architecture:** Un server component unique appelle `getCustomer()` pour récupérer toutes les données client, puis `getCustomerById()` pour le nom du parrain et `getCustomersByIds()` (nouvelle fonction) pour les noms des filleuls. Tout est rendu côté serveur en sections empilées.

**Tech Stack:** Next.js 15 App Router, TypeScript strict, Tailwind CSS, Supabase

**Spec:** `docs/superpowers/specs/2026-04-14-module5-customer-detail-design.md`
...
|---------|--------|----------------|
| `lib/db/customers.ts` | Modifier | Ajouter `getCustomersByIds(ids)` |
| `app/(authenticated)/customers/[id]/page.tsx` | Créer | Fiche client complète (server component) |

---

### Task 1: Ajouter getCustomersByIds dans lib/db/customers.ts

**Files:**
- Modify: `lib/db/customers.ts`

- [ ] **Step 1: Ajouter la fonction après getRecentCustomers**

Ajouter cette fonction après `getRecentCustomers` dans `lib/db/customers.ts` :
...
Note: `CustomerListItem` is already exported from this file (added in Module 3).

- [ ] **Step 2: Commit**

```bash
git add lib/db/customers.ts
git commit -m "feat: add getCustomersByIds for batch customer lookup"
```

---

### Task 2: Créer la page fiche client

**Files:**
- Create: `app/(authenticated)/customers/[id]/page.tsx`
...
Expected: `✓ Compiled successfully` with `/customers/[id]` route visible

- [ ] **Step 3: Commit**

```bash
git add "app/(authenticated)/customers/[id]/page.tsx"
git commit -m "feat: create customer detail page with all sections"
```

---

### Task 3: Vérification end-to-end
...
|----------|-------------------|
| `/customers/[id-valide]` | Fiche client complète avec toutes les sections |
| `/customers/[id-inexistant]` | Page 404 |
| Client sans ventes | Section "Aucune vente enregistrée" |
| Client sans filleuls | Section "Aucun filleul" |
| Client sans bons | Section "Aucun bon d'achat" |
| Client parrainé | Lien cliquable vers la fiche du parrain |
| Client avec filleuls | Noms cliquables vers les fiches filleuls |
| Client avec bons disponibles | Bouton "Utiliser un bon" visible |
| Client sans bons disponibles | Pas de bouton "Utiliser un bon" |
| Bouton "Saisir une vente" | Lien vers `/customers/[id]/new-sale` |
| Lien "← Retour aux clients" | Retour à `/customers` |
| Clic depuis la liste clients | Navigation correcte vers la fiche |
