# Module 4 — Création de client Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Créer la page `/customers/new` avec un formulaire de création de client (react-hook-form + Zod) incluant une recherche de parrain optionnelle avec debounce.

**Architecture:** Une page server component minimaliste rend un client component `CreateCustomerForm` qui gère toute l'interactivité : validation Zod via react-hook-form, recherche de parrain debounced via `searchCustomers` server action, soumission via `createCustomer` server action, et redirection vers la fiche client au succès.

**Tech Stack:** Next.js 15 App Router, TypeScript strict, Tailwind CSS, react-hook-form, @hookform/resolvers, Zod

**Spec:** `docs/superpowers/specs/2026-04-14-module4-create-customer-design.md`
...
|---------|--------|----------------|
| `package.json` | Modifier | Ajouter @hookform/resolvers |
| `app/(authenticated)/customers/new/page.tsx` | Créer | Page server : titre + rendu du formulaire |
| `components/customers/CreateCustomerForm.tsx` | Créer | Formulaire complet avec validation + recherche parrain |

---

### Task 1: Installer @hookform/resolvers
...
git commit -m "chore: add @hookform/resolvers for Zod form validation"
```

---

### Task 2: Créer le composant CreateCustomerForm

**Files:**
- Create: `components/customers/CreateCustomerForm.tsx`
...
```bash
git add components/customers/CreateCustomerForm.tsx
git commit -m "feat: create CreateCustomerForm with validation and referrer search"
```

---

### Task 3: Créer la page /customers/new

**Files:**
- Create: `app/(authenticated)/customers/new/page.tsx`
...
Expected: `✓ Compiled successfully` with `/customers/new` route visible

- [ ] **Step 3: Commit**

```bash
git add "app/(authenticated)/customers/new/page.tsx"
git commit -m "feat: create new customer page"
```
...
|----------|-------------------|
| Naviguer vers `/customers/new` | Formulaire avec 4 champs + checkbox parrain + boutons |
| Soumettre vide | Erreurs Zod sous chaque champ en rouge |
| Remplir les champs correctement sans parrain | Création réussie → redirigé vers `/customers/[id]` |
| Cocher "Ce client a un parrain" | Champ de recherche apparaît |
| Chercher un parrain existant | Suggestions affichées après debounce |
| Sélectionner un parrain | Badge bleu avec nom + bouton ✕ |
| Cliquer ✕ sur le badge | Parrain retiré, recherche réapparaît |
| Décocher la checkbox | Section parrain disparaît, referrerId reset |
| Créer un client avec email existant | Erreur serveur "Ce client existe déjà" en bannière rouge |
| Cliquer "Annuler" | Retour à `/customers` |
